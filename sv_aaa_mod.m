function [r, pol, res, zer, z, f, w, errvec, zind] = sv_aaa_mod(F, varargin)
% YN modified to remove chol trick, as I suspect it's losing accuracy.
%SV_AAA - Set-valued AAA for computing a rational approximation of many
%         functions on a discrete domain.
%
...
% This is a slightly modified version from autoCORK [1] by Lietaert et.al.,
...
% Parse inputs:
[F, Z, M, nF, dom, tol, mmax, needZ, mmax_flag, scale_each_F] = parseInputs(F, varargin{:});
...
% Scale the functions
%%% MODIFICATION - BV - START
% The original SV_AAA assumed a split form F(z) = sum_i f_i(z) A_i
% and approximated the functions {f_1, ..., f_s}. Each f_i was scaled to
% have norm 1 on the sampling set.
% Instead, we can also approximate all the entries in F(z) as individual
% functions. However, scaling does not work in general since in many
% problems some entries of F(z) are always zero.
if scale_each_F % scaling to get split functions on same magnitude, 
                % does not work if F has zeros
    normF = max(abs(F),[],1);
else
    normF = max(abs(F(:)));    
end
...
%%% MODIFICATION - BV - START
% Left scaling matrix:  %%% This really slow for large problems. Avoided below.
...
F = F(:);
R = mean(F);
errvec = zeros(mmax+1,1);
z = zeros(mmax,1);
f = zeros(mmax,nF);  %%%% BIG!
ind = zeros(mmax,nF);  %%%% BIG!
H = zeros(mmax,mmax-1);
S = zeros(mmax,mmax-1);
...
for m = 1:mmax
    ...
    C(ind(1:m,1),1:m) = 0;                          % Set the selected elements to 0
    
    %{
    v = C(:,m)*f(m,:);                            % Compute the next vector of the basis.
    %%% MODIFICATION - BV - START
    % v = SF*C(:,m)-v(:);
    % much faster than SF*C(:,m) since it avoids the construction of the costly SF
    v = F .* repmat(C(:,m),[nF, 1]) - v(:);
    %%% MODIFICATION - BV - END
    ...
    A = []; % slow but steady!
    for mnow = 1:m
    Aadd = F .* repmat(C(:,mnow),[nF, 1]); 
    Aminus = []; 
    for nFnow = 1:nF
    Aminus = [Aminus;C(:,mnow)*f(mnow,nFnow)];
    end
    A = [A Aadd-Aminus];        
    end    
    [~,~,V] = svd(A,'econ');
...
z = z(1:m);
%%% MODIFICATION - BV - START
errvec = errvec(2:m+1);  % errvec(1) is for constant approx, so start from 2
                         % also, no multiplication by normF since we want
                         % relative
%%% MODIFICATION - BV - END
...
%%% MODIFICATION - BV - START
% Disabled since it takes a lot of time
...
function [F, Z, M, nF, dom, tol, mmax, needZ, mmax_flag, scale_each_F] = ...
...
while ( ~isempty(varargin) )
    ...
    elseif ( strncmpi(varargin{1}, 'scale_each_F', 3) )
        if ( islogical(varargin{2}) )
            scale_each_F = varargin{2};
        end
        varargin([1, 2]) = [];
...
% BV: put in the file reval.m -> done by YN here
